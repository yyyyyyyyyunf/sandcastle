...
AFK-P6 separates loop progression from this shutdown hint in prepared runs. A COMPLETE marker can start shutdown grace, but only an accepted candidate returns to the next host preparation. Held/invalid output cannot become success through a completion token. The complete output buffer, including trailing SIGTERM-handler output, is sealed before per-iteration extraction.
